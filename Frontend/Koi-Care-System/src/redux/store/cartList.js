...
export const addToCartList = (product, quantity) => async (dispatch) => {
  ...
  try {
    ...
    await axios.post(
      'https://koicaresystemv2.azurewebsites.net/api/cartItems/item/add',
      {
        cartId: cartId,
...
export const removeToCartList = (productId) => async (dispatch) => {
  ...
  try {
    ...
    const response = await axios.delete(
      `https://koicaresystemv2.azurewebsites.net/api/cartItems/cart/${cartId}/product/${productId}/remove`,
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
