...
function CustomerAD() {
  ...
  const getUser = async () => {
    try {
      ...
      const res = await axios.get(`https://koicaresystemv4.azurewebsites.net/api/profile/all`, {
        headers: {
  ...
  const upgradeSubscription = async (userId, period, role) => {
    ...
    try {
      ...
      const response = await axios.put('https://koicaresystemv4.azurewebsites.net/api/subscribe/upgrade', data, {
        headers: {
          Authorization: `Bearer ${token}`,
