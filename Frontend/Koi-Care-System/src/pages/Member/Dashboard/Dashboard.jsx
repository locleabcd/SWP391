...
function Dashboard() {
  ...
  const getPayments = async () => {
    try {
      ...
      const res = await axios.get(`https://koicaresystemv2.azurewebsites.net/api/payment/user/${userId}`, {
        headers: {
  ...
  const getReport = async () => {
    try {
      ...
      for (const date of surroundingDates) {
        const res = await axios.get('https://koicaresystemv2.azurewebsites.net/api/reports/FishPondWater', {
          headers: {
            Authorization: `Bearer ${token}`
          },
          params: {
