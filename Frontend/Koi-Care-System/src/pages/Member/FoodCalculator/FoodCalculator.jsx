...
function FoodCalculator() {
  ...
  const calculateFoodAmountBySize = useCallback((total) => {
    let minMultiplier = 1; // Minimum percentage
    let maxMultiplier = 1; // Maximum percentage

    if (selectedSize === 'low') {  // Small Fish
    ...
    if (selectedSize === 'high') {  // Large Fish
        if (selectedTemperature === '6-8') {
            minMultiplier = 0.003; 
            maxMultiplier = 0.005;
        }
  ...
  const calculateFoodAmountByPercent = useCallback((total) => {
...
}, [fishes, currentPercent, selectedSize, selectedTemperature, calculationMode, calculateFoodAmountByPercent, calculateFoodAmountBySize]);
          ...
          <div className='py-5 px-[30px] mx-auto'>
            ...
            <div className="p-4 text-lg">
              <label htmlFor="ponds">Select a Pond:</label>
                ...
                >
                  <option value="all" disabled>All Ponds</option>
                  {ponds.map((pond) => (
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className={`px-4 py-2 rounded ${selectedSize === 'low' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-blue-500'}`}
                        >
                          Low
                        </button>
                        <button
                          ...
                          className={`px-4 py-2 rounded ${selectedSize === 'mid' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-blue-500'} ${isSizeDisabled('mid') ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                          Mid
                        </button>
                        <button
                          ...
                          className={`px-4 py-2 rounded ${selectedSize === 'high' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-blue-500'} ${isSizeDisabled('high') ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                          High
                        </button>
                      </div>
                        ...
                        <div className="flex space-x-2">
                          {temperatureOptions.map((temp) => (
                            <button
                              ...
                              className={`px-4 py-2 rounded ${selectedTemperature === temp ? 'bg-blue-500 text-white' : 'bg-gray-200 text-blue-500  '}`}
                            >
                              {temp}
                            </button>
                          ))}
            ...
            <div className="footer w-2/4 bg-slate-600 text-white border-solid rounded-lg mx-auto py-5 text-xl text-center">
              {fishes.length > 0 ? (
                  calculationMode === 'percent' ? (
                      <p>Food Amount: <strong>{!isNaN(foodAmount) ? foodAmount.toFixed(2) : 0}g</strong><strong>/Perday</strong></p> 
                  ) : (
                    <p>Food Amount: <strong>
                    {!isNaN(foodAmount.min) ? foodAmount.min.toFixed(2) : 0}g - {!isNaN(foodAmount.max) ? foodAmount.max.toFixed(2) : 0}g<strong>/Perday</strong>
                  </strong></p>
                  )
              ) : (
                  <p>Total Fish Weight: <strong>0g</strong></p>
