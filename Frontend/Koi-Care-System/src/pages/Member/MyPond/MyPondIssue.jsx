...
function MyPondIssue() {
  ...
  const getProduct = async (issueTypeId) => {
    try {
      const token = localStorage.getItem('token')
      const res = await axios.get(`https://koicaresystemv2.azurewebsites.net/api/products/recommend/${issueTypeId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
  ...
  const handleChange = (categoryId) => {
    ...
    const currentIssue = issue.find((item) => item.id === categoryId)
              ...
              <div className='flex-auto'>
                <div className='flex flex-col lg:flex-row'>
                  <div className='flex-none w-full lg:w-[800px] px-4 py-7 lg:px-7 lg:py-7 border-r border-gray-200'>
                    <div className='font-semibold text-xl lg:text-2xl'>Products</div>
                    <motion.div
                      initial='hidden'
                      animate='visible'
                      variants={{
                      ...
                      className='grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-8 py-3 mt-4'
                    >
                      {product.length > 0 ? (
                  ...
                  <div className='flex-auto px-4 py-7 lg:px-7 lg:py-7'>
                    <div className='text-start font-semibold text-xl lg:text-2xl'>Description</div>
                    <div className='text-lg lg:text-2xl mt-5 text-justify'>
                      {selectedIssue ? selectedIssue.description : 'No description available'}
                    </div>
                  </div>
                </div>
              </div>
