...
function Supplier() {
  ...
  const deleteSupplier = async (id) => {
    ...
    try {
      ...
      await axios.delete(`https://koicaresystem.azurewebsites.net/api/suppliers/supplier/${id}/delete`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
                    ...
                    <tr key={supplier.id}>
                      ...
                      <td className='py-2 px-1 text-center border-b border-gray-200'>
                        <div className='flex justify-center items-center'>
                          <Link to={`/shop/supplier/${supplier.id}`} className='p-1 hover:bg-green-500 text-green-500 hover:text-white rounded-full'>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              viewBox="0 0 24 24" 
                              fill="currentColor" 
                          ...
                          <button className='p-1 hover:bg-red-500 text-red-600 hover:text-white rounded-full' onClick={() => deleteSupplier(supplier.id)}>
                            <svg 
                              xmlns="http://www.w3.org/2000/svg" 
                              viewBox="0 0 24 24" 
                              fill="currentColor" 
